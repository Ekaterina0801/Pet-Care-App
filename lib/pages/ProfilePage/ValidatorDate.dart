...
String validateDatewithPet(String value, String petBirth) {
  String _msg;
 RegExp regex = new RegExp(r'^(\d{4})-(\d{2})-(\d{2})$');
  var match = regex.firstMatch(value);
   var year = int.parse(match.group(1));
  ...
  if (value.isEmpty) {
    ...
    _msg = "Введите день в диапазоне [01;28]";
      ...
      (year == DateTime.now().year &&
          ...
          day > DateTime.now().day))
    _msg = "Эта дата ещё не наступила";
  else if ((year < int.parse(petBirth.substring(0, 4))) ||
      (year == int.parse(petBirth.substring(0, 4)) &&
          month < int.parse(petBirth.substring(8, 10))) ||
      (year == int.parse(petBirth.substring(0, 4)) &&
          month == int.parse(petBirth.substring(8, 10)) &&
          day < int.parse(petBirth.substring(5, 7))))
    _msg = "Эта дата была раньше рождения вашего питомца";
  return _msg;
}


String validateDatewithPetSecond(String value, String second_date) {
  String _msg;
 RegExp regex = new RegExp(r'^(\d{4})-(\d{2})-(\d{2})$');
  var match = regex.firstMatch(value);
   var year = int.parse(match.group(1));
  ...
  if (value.isEmpty) {
    ...
    _msg = "Введите день в диапазоне [01;28]";
      ...
      (year == DateTime.now().year &&
          ...
          day > DateTime.now().day))
    _msg = "Эта дата ещё не наступила";
 else if ((year < int.parse(second_date.substring(0, 4))) ||
      (year == int.parse(second_date.substring(0, 4)) &&
          month < int.parse(second_date.substring(8, 10))) ||
      (year == int.parse(second_date.substring(0, 4)) &&
          month == int.parse(second_date.substring(8, 10)) &&
          day < int.parse(second_date.substring(5, 7))))
    _msg = "Эта дата была раньше начала болезни";
  return _msg;
}
